  ...
  ({
    ...
    return (
      ...
      >
        ...
        />
        <Box
          sx={{ display: "flex", alignItems: "center", gap: 1, mt: 0.5, ml: 1 }}
        >
          <Chip
...
const UploadSection = () => {
  ...
  const handleOpenConfirm = useCallback(() => {
    if (files.length === 0 || files.some((_, idx) => !fileTemplates[idx])) {
      setSnackbar({
        open: true,
        message: "Debes seleccionar una plantilla para cada archivo.",
        severity: "error",
      });
      return;
    }
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err.response?.data?.message || "Error al subir los archivos.";
  ...
  return (
    ...
    >
      <CardContent sx={{ p: 4 }}>
        ...
        >
          Sube uno o varios archivos .xls o .xlsx para actualizar los datos de
          análisis de todas las secretarías.
        </Typography>
          ...
          <DialogContent>
            ...
            <List dense>
              {files.slice(0, 5).map(
                (
                  file,
                  idx, // Limitamos a 5 para mejor rendimiento
                ) => (
