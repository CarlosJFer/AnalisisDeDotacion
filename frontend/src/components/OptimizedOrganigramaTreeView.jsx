...
const TreeNode = memo(
  ({
    ...
    const tooltipContent = useMemo(() => {
      ...
      const variablesAlerta = variables.filter(
        (v) => v.estado === "critico" || v.estado === "preventivo",
      );
    ...
    const funcionTooltip = useMemo(() => {
      return node.funcion && node.funcion.trim()
        ? node.funcion
        : "Sin descripciÃ³n de funciones";
    }, [node.funcion]);
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.25 }}>
              ...
              {node.nombre !== "Municipalidad" && (
                <>
                  {onNodeSelect && (
                    <Tooltip
                      title="Ver dashboard planta y contratos"
                      placement="top"
                      enterDelay={300}
                    >
                      <IconButton
                  ...
                  {onNodeSelectNeike && (
                    <Tooltip
                      title="Ver dashboard neikes y beca"
                      placement="bottom"
                      enterDelay={300}
                    >
                      <IconButton
...
const OptimizedOrganigramaTreeView = forwardRef(
  (
    { tree, getVariablesForNode, searchTerm, onNodeSelect, onNodeSelectNeike },
    ref,
  ) => {
