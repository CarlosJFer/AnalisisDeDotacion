...
const ExportOptions = ({ data, secretariaId, widgetId = null }) => {
  ...
  const exportToPDF = async () => {
    try {
      ...
      const element =
        document.querySelector('[data-tour="widget-grid"]') || document.body;
      ...
      // Añadir título
      pdf.setFontSize(16);
      pdf.text("Análisis de Dotación de Personal", pdfWidth / 2, 20, {
        align: "center",
      });

      if (data?.secretaria?.nombre) {
        pdf.setFontSize(12);
        pdf.text(`Secretaría: ${data.secretaria.nombre}`, pdfWidth / 2, 25, {
          align: "center",
        });
      }
      ...
      if (exportOptions.includeData && data) {
        ...
        if (data.resumen) {
          ...
          yPos += 6;
          pdf.text(
            `Masa Salarial: $${data.resumen.masaSalarial?.toLocaleString("es-AR")}`,
            20,
            yPos,
          );
          yPos += 6;
          pdf.text(
            `Sueldo Promedio: $${data.resumen.sueldoPromedio?.toLocaleString("es-AR")}`,
            20,
            yPos,
          );
          yPos += 15;
        ...
        if (data.analisis?.contratacion) {
          ...
          data.analisis.contratacion.forEach((item) => {
            pdf.text(
              `${item.tipo}: ${item.cantidad} (${item.porcentaje}%)`,
              25,
              yPos,
            );
            yPos += 6;
    ...
    try {
      ...
      if (data?.analisis?.contratacion) {
        const contractData = [
          ...
          ...data.analisis.contratacion.map((item) => [
            item.tipo,
            item.cantidad,
            item.porcentaje,
          ]),
      ...
      if (data?.analisis?.antiguedad) {
        const ageData = [
          ...
          ...data.analisis.antiguedad.map((item) => [
            item.rango,
            item.cantidad,
            item.porcentaje,
          ]),
    ...
    <>
      ...
      >
        {formatOptions.map((format) => (
          <MenuItem
            key={format.value}
            onClick={() => openExportDialog(format.value)}
          >
