...
const CustomDonutChart = React.memo(
  ({ data, title, isDarkMode, dataKey, nameKey, height = 400 }) => {
    ...
    const CustomTooltip = ({ active, payload }) => {
      if (active && payload && payload.length) {
        ...
        const percentage =
          total > 0 ? ((data[dataKey] / total) * 100).toFixed(1) : 0;
        return (
          <Box
            sx={{
          ...
          >
            ...
            <Typography variant="body2">Porcentaje: {percentage}%</Typography>
          </Box>
        );
      }
      return null;
