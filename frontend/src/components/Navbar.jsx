...
const Navbar = () => {
  ...
  return (
    <AppBar position="static" sx={{ backgroundColor: 'transparent', boxShadow: 'none', borderBottom: '1px solid rgba(0, 0, 0, 0.1)' }}>
      <Toolbar sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <Box className="flex flex-row items-center gap-2" sx={{ marginLeft: 'auto' }}>
          ...
          {user && (
            <Button component={Link} to="/organigrama" sx={{ color: '#333' }}>Inicio</Button>
          )}
          {/* Botón Dashboard */}
          {user && (
            <Button component={Link} to={user.role === 'admin' ? '/dashboard/default' : '/dashboard/default'} sx={{ color: '#333' }}>Dashboard</Button>
          )}
          {/* Botón Panel de administración (solo admin) */}
          {user && user.role === 'admin' && (
            <Button component={Link} to="/admin" sx={{ color: '#333' }}>Panel de administración</Button>
          )}
          {user ? (
            <>
              <Typography component="span" sx={{ mr: 1, ml: 2, color: '#333' }}>
              ...
              <Tooltip title={unreadCount > 0 ? `Tienes ${unreadCount} notificaciones nuevas` : 'Sin notificaciones nuevas'}>
                <IconButton sx={{ color: '#333' }} onClick={handleOpen}>
              ...
              {/* Selector de tema */}
              <IconButton sx={{ color: '#333' }} onClick={toggleTheme} title={isDarkMode ? 'Tema claro' : 'Tema oscuro'}>
                {isDarkMode ? <Brightness7Icon /> : <Brightness4Icon />}
              </IconButton>
              <Button onClick={logout} sx={{ color: '#333' }}>Salir</Button>
            </>
          ) : (
            null
          )}
