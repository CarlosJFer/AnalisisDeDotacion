...
const getAncestors = (list, node) => {
  ...
  while (current && current.idPadre) {
    const parent = list.find(
      (dep) => String(dep._id) === String(current.idPadre),
    );
    if (parent) {
...
const OrganigramaPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError(
          "No se pudo cargar el Ã¡rbol de dependencias, variables o valores actuales",
        );
      } finally {
        setLoading(false);
      }
  ...
  const getVariablesForNode = useCallback(
    (node) => {
      ...
      const valuesForDep = variableValues.filter(
        (vv) =>
          vv.dependenciaId &&
          (vv.dependenciaId._id === node._id || vv.dependenciaId === node._id),
      );
      // Mapear cada variable con su valor actual (si existe)
      return variables.map((v) => {
        const valueObj = valuesForDep.find(
          (val) =>
            val.variableId &&
            (val.variableId._id === v._id || val.variableId === v._id),
        );
        ...
        if (valor_actual !== null) {
          if (v.flexible) {
            ...
            else if (valor_actual < pInf || valor_actual > pSup)
              estado = "preventivo";
          } else {
  ...
  const filteredTree = useMemo(() => {
    ...
    const filteredList = flatList.filter((dep) =>
      requiredIds.has(String(dep._id)),
    );
  ...
  const handleExpandAll = () => {
    if (treeViewRef.current && treeViewRef.current.expandAll)
      treeViewRef.current.expandAll();
  };
  const handleCollapseAll = () => {
    if (treeViewRef.current && treeViewRef.current.collapseAll)
      treeViewRef.current.collapseAll();
  };
      ...
      {!loading &&
        ...
        (filteredTree.length === 0 && search ? (
          ...
          <OrganigramaTreeView
            ...
            onNodeSelect={(node) =>
              navigate("/dashboard", {
                state: { nombre: node.nombre, nivel: node.nivel },
              })
            }
            onNodeSelectNeike={(node) =>
              navigate("/dashboard-neike-beca", {
                state: { nombre: node.nombre, nivel: node.nivel },
              })
            }
          />
